...
void launchMetalWrap(BufferParam output, size_t outputBytes,
...
void wrapMetal(Param<T> output, CParam<T> input, const dim_t windowX,
    ...
    launchMetalWrap(output.bufferParam(), outputElements * sizeof(T),
                    output.dims(), output.strides(), input.bufferParam(),
                    inputElements * sizeof(T), input.dims(), input.strides(),
                    windowX, windowY, strideX, strideY, paddingX, paddingY,
                    dilationX, dilationY,
