...
void launchMetalIir(void* output, size_t outputBytes,
                    const af::dim4& outputDims, const af::dim4& outputStrides,
                    const void* coefficients, size_t coefficientBytes,
...
void iirMetal(Param<T> output, Param<T> coefficients, CParam<T> feedback) {
    ...
    launchMetalIir(
        ...
        output.dims(), output.strides(), coefficients.get(),
        coefficientElements * sizeof(T), coefficients.strides(), feedback.get(),
        feedbackElements * sizeof(T), feedback.dims(), feedback.strides(),
        feedbackBatched, static_cast<af_dtype>(af::dtype_traits<T>::af_type));
}
