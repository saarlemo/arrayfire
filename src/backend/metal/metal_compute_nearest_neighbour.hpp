...
void launchMetalNearestNeighbour(
    ...
    const af::dim4& outputStrides, const void* query, size_t queryBytes,
    const af::dim4& queryDims, const af::dim4& queryStrides, const void* train,
    size_t trainBytes, const af::dim4& trainDims, const af::dim4& trainStrides,
    unsigned distanceDimension, af_match_type distanceType, af_dtype inputType,
    af_dtype outputType);
...
void nearestNeighbourMetal(Param<To> output, CParam<T> query, CParam<T> train,
    ...
    launchMetalNearestNeighbour(
        ...
        output.dims(), output.strides(), query.get(), queryElements * sizeof(T),
        query.dims(), query.strides(), train.get(), trainElements * sizeof(T),
        train.dims(), train.strides(), distanceDimension, distanceType,
