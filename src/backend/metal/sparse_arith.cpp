    ...
    switch (out.getStorage()) {
        case AF_STORAGE_CSR:
            if constexpr (std::is_same<T, float>::value) {
                getQueue().enqueue(kernel::sparseArithMetal, out.getValues(),
                                   out.getRowIdx(), out.getColIdx(), rhs, true,
                                   reverse,
                                   op == af_add_t   ? 0U
                                   : op == af_sub_t ? 1U
                                   : op == af_mul_t ? 2U
                ...
                getQueue().enqueue(
                    kernel::sparseArithOpS<T, op, AF_STORAGE_CSR>,
                    out.getValues(), out.getRowIdx(), out.getColIdx(), rhs,
                    reverse);
            }
            break;
        case AF_STORAGE_COO:
            if constexpr (std::is_same<T, float>::value) {
                getQueue().enqueue(kernel::sparseArithMetal, out.getValues(),
                                   out.getRowIdx(), out.getColIdx(), rhs, false,
                                   reverse,
                                   op == af_add_t   ? 0U
                                   : op == af_sub_t ? 1U
                                   : op == af_mul_t ? 2U
                ...
                getQueue().enqueue(
                    kernel::sparseArithOpS<T, op, AF_STORAGE_COO>,
                    out.getValues(), out.getRowIdx(), out.getColIdx(), rhs,
                    reverse);
            }
            break;
